...
const SpecularButton = ({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`relative m-0 inline-flex cursor-pointer items-center justify-center gap-2 border-none font-medium leading-none tracking-[0.01em] outline-none transition-[transform,background-color] duration-200 ease-out active:scale-[0.97] disabled:cursor-default disabled:opacity-55 disabled:active:scale-100 [color:var(--sb-text-color)] [border-radius:var(--sb-radius)] [background:color-mix(in_srgb,var(--sb-tint)_calc(var(--sb-tint-opacity)*100%),transparent)] [backdrop-filter:blur(var(--sb-blur))] focus-visible:outline-2 focus-visible:outline-offset-[3px] ${SIZES[size] || SIZES.md}${className ? ` ${className}` : ''}`}
      style={
        {
          '--sb-radius': `${radius}px`,
          '--sb-tint': tint,
    ...
    >
      ...
      <span className="relative z-[2] inline-flex items-center gap-2 [&_svg]:h-[1.1em] [&_svg]:w-[1.1em] [&_svg]:shrink-0">{children}</span>
    </button>
  );
};
